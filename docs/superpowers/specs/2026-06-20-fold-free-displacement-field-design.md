...
### Verification result (2026-06-20, browser pixel-Jacobian on the real emitted SVGs)

`verify/fold-check.html` rasterizes the actual emitted maps (generated by `src/__tests__/_genFoldMaps.test.ts`)
and measures the Jacobian min-det:

- **Legacy (`shapeAdapt:false`) folds at every scale and grid resolution** — `sq320`/`wide480` min-det
  `−9e-4 … −3.3e-3` (a real, visible geometric fold, robust across grid `n = 48…220`).
- **New (`shapeAdapt:true`) eliminates it** — min-det at the 8-bit quantization floor (`+5.6e-5 … −3.5e-5`,
  ~30–90× smaller than legacy at matched resolution). The **float model** (the Task 2 jest sweep, which
  includes `[480,160]@480`) proves the *geometry* is injective; the tiny raster residual is **8-bit
  quantization noise of the displacement map itself** (it flips sign with grid resolution and gets
  *worse* with more attenuation — both signatures of quantization, not geometry), which is unavoidable
  for any 8-bit displacement map and sub-perceptual.
- **Calibration confirmed `BAND_K = 1.5` is optimal** (envelope blur `W·0.5`); wider blur and higher
  `BAND_K` did not improve the genuine cases and slightly worsened the quantization floor. **The
  conditional radial-corner reconstruction (mechanism 3) was therefore NOT needed** — it addresses
  geometry, and the geometry is already fold-free.

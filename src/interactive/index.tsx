...
import React, { useEffect, useRef, useState, useImperativeHandle, forwardRef, type RefObject } from 'react';
...
export const LiquidGlassInteractive = forwardRef<LiquidGlassHandle, LiquidGlassInteractiveProps>(
  function LiquidGlassInteractive(
    ...
    const bumpId = stableId ? `lg-bump-${stableId}` : undefined;
    ...
    useEffect(() => {
      ...
      if (!el || (typeof window.matchMedia === 'function' && window.matchMedia('(prefers-reduced-motion: reduce)').matches)) return;
